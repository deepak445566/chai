      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
        ...
        >
          ...
          <div className="relative mb-2">
                ...
                <h1 className='text-9xl ab  '>Launching Soon</h1>
            ...
            <div className="relative mb-8">
              ...
              >
                ...
                <div className="relative mx-auto max-w-xs sm:max-w-sm">
                 ...
                 {/* Pure Chai Cup - Top View Without Spices */}
<div className="relative w-64 h-64 sm:w-80 sm:h-80 rounded-full shadow-2xl overflow-hidden border-4 border-amber-300/40 mx-auto">
  
  {/* Cup Rim - Thick Border */}
  <div className="absolute inset-0 rounded-full border-8 border-amber-200/50"></div>
  
  {/* Cup Inner Shadow */}
  <div className="absolute inset-2 rounded-full bg-gradient-to-b from-amber-900/10 to-transparent"></div>
  
  {/* Chai Liquid - Pure Tea */}
  <div className="absolute inset-4 rounded-full bg-gradient-to-b from-amber-900 via-amber-800 to-amber-950 overflow-hidden">
    
    {/* Milk Layer on Top */}
    <div className="absolute top-0 left-0 right-0 h-1/3 bg-gradient-to-b from-amber-100/50 via-amber-50/30 to-transparent rounded-t-full">
      
      {/* Cream Foam Center */}
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-16 h-16 rounded-full bg-gradient-to-b from-amber-50/60 to-amber-100/30 blur-sm"></div>
      
      {/* Milk Swirl Pattern */}
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        {/* Concentric Milk Rings */}
        {[1, 2, 3].map((ring) => (
          <div
            key={`milk-ring-${ring}`}
            className="absolute rounded-full border border-amber-50/20 blur-xs"
            style={{
              width: `${ring * 24}px`,
              height: `${ring * 24}px`,
              top: '50%',
        ...
        <div className="relative">
          {[...Array(6)].map((_, i) => {
            ...
            return (
              <div
                key={`milk-drop-${i}`}
                className="absolute w-4 h-4 bg-amber-50/30 rounded-full blur-xs"
                style={{
                  left: `${x}px`,
                  top: `${y}px`,
                  animation: `pulse ${2 + i * 0.3}s infinite ${i * 0.2}s`
                }}
              ></div>
            );
          })}
      ...
      <div className="absolute inset-0">
        {[...Array(12)].map((_, i) => {
          ...
          return (
            <div
              key={`bubble-${i}`}
              className="absolute rounded-full bg-amber-50/20"
              style={{
                top: `${y}%`,
                left: `${x}%`,
                width: `${size}px`,
                height: `${size}px`,
                transform: 'translate(-50%, -50%)',
                animation: `float ${3 + Math.random() * 2}s infinite ease-in-out ${i * 0.1}s`
              }}
            ></div>
          );
        })}
    ...
    {/* Pure Tea Layer - Dark Rich Color */}
    <div className="absolute inset-0 bg-gradient-to-b from-transparent via-amber-900/80 to-amber-950 rounded-full">
      
      {/* Tea Texture - Subtle Variations */}
      <div className="absolute inset-0">
        {/* Tea Concentric Rings */}
        <div className="absolute inset-4 rounded-full border-2 border-amber-900/40"></div>
        <div className="absolute inset-8 rounded-full border border-amber-800/30"></div>
        
        {/* Tea Streaks/Currents */}
        <div className="absolute top-1/4 left-1/4 w-16 h-1 bg-gradient-to-r from-transparent via-amber-800/30 to-transparent transform rotate-45 blur-xs"></div>
        <div className="absolute top-1/3 right-1/4 w-12 h-1 bg-gradient-to-r from-transparent via-amber-800/25 to-transparent transform -rotate-30 blur-xs"></div>
        <div className="absolute bottom-1/3 left-1/3 w-20 h-1 bg-gradient-to-r from-transparent via-amber-800/20 to-transparent transform rotate-15 blur-xs"></div>
        
        {/* Tea Sediment/Depth */}
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 w-3/4 h-8 bg-gradient-to-t from-amber-950 to-transparent rounded-full"></div>
      </div>
      ...
      <div className="absolute inset-0">
        {[1, 2, 3].map((ripple) => (
          <div
            key={`ripple-${ripple}`}
            className="absolute rounded-full border border-amber-800/15"
            style={{
              margin: `${ripple * 6}px`,
              animation: `ripple ${4 + ripple * 0.5}s infinite linear ${ripple * 0.3}s`
            }}
          ></div>
        ))}
      </div>
    ...
    {/* Steam Rising from Pure Chai */}
    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
      <div className="relative">
        {/* Main Steam Column */}
        <div className="w-12 h-12 bg-gradient-to-b from-amber-50/50 via-amber-100/30 to-transparent rounded-full blur-md animate-pulse"></div>
        
        {/* Expanding Steam Rings */}
        {[1, 2, 3].map((ring) => (
          <div
            key={`steam-ring-${ring}`}
            className="absolute rounded-full border border-amber-100/20 blur-sm"
            style={{
              width: `${ring * 32}px`,
              height: `${ring * 32}px`,
              top: '50%',
              left: '50%',
              transform: 'translate(-50%, -50%)',
              animation: `steamExpand ${1.5 + ring * 0.3}s infinite ease-out ${ring * 0.2}s`
            }}
          ></div>
        ))}
        ...
        <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
          {[...Array(3)].map((_, i) => (
            <div
              key={`steam-wisp-${i}`}
              className="absolute w-6 h-4 bg-gradient-to-b from-amber-50/30 to-transparent rounded-full blur-sm"
              style={{
                left: `${(i - 1) * 8}px`,
                transform: `rotate(${i * 30 - 30}deg)`,
                animation: `wispFloat ${2 + i * 0.2}s infinite ${i * 0.3}s`
              }}
            ></div>
          ))}
        </div>
    ...
    {/* Tea Temperature Gradient Rings */}
    <div className="absolute inset-0">
      <div className="absolute inset-2 rounded-full bg-gradient-to-b from-amber-700/10 to-transparent"></div>
      <div className="absolute inset-6 rounded-full bg-gradient-to-b from-amber-800/5 to-transparent"></div>
    </div>
  </div>
  
  {/* Cup Handle - Top View */}
  <div className="absolute -right-8 top-1/2 transform -translate-y-1/2 w-10 h-36 bg-gradient-to-r from-amber-300/50 to-amber-400/40 rounded-l-2xl shadow-xl">
    {/* Handle Details */}
    <div className="absolute inset-y-4 left-3 w-1 bg-gradient-to-b from-amber-400/60 to-amber-500/50 rounded-full"></div>
    <div className="absolute top-1/2 left-2 w-6 h-1 bg-gradient-to-r from-amber-400/40 to-transparent transform -translate-y-1/2 rotate-90"></div>
  </div>
  
  {/* Cup Shadow/Depth */}
  <div className="absolute -inset-3 rounded-full bg-gradient-to-br from-amber-900/30 via-transparent to-amber-900/30 blur-lg"></div>
  
  {/* Cup Base Shadow */}
  <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 w-4/5 h-6 bg-gradient-to-t from-amber-900/25 to-transparent rounded-full blur-md"></div>
  
  {/* Cup Gloss/Highlight */}
  <div className="absolute top-4 right-6 w-12 h-12 bg-gradient-to-br from-white/10 to-transparent rounded-full blur-sm"></div>
  <div className="absolute bottom-6 left-8 w-8 h-8 bg-gradient-to-br from-white/5 to-transparent rounded-full blur-sm"></div>
</div>
...
`}</style>
                  ...
                  <div className="absolute bottom-10 -left-24">
                    <div 
                      ...
                      className="w-14 h-14 sm:w-29 sm:h-29 rounded-full   flex items-center justify-center"
                    >
