...
<div className="relative w-64 h-64 sm:w-80 sm:h-80 rounded-full shadow-2xl overflow-hidden border-4 border-amber-300/40 mx-auto">
  ...
  {/* Cup Rim */}
  <div className="absolute inset-0 rounded-full border-6 border-amber-200/50"></div>
  
  {/* Chai Liquid */}
  <div className="absolute inset-6 rounded-full bg-gradient-to-b from-amber-800 via-amber-700 to-amber-900">
    
    {/* Milk Foam Center */}
    <div className="absolute inset-8 rounded-full bg-gradient-to-b from-amber-100/60 to-amber-50/30">
      
      {/* Central Spice Cluster */}
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex items-center justify-center">
        {/* Cardamom Cluster */}
        <div className="relative">
          <div className="w-8 h-8 bg-gradient-to-r from-green-900/70 to-green-800/60 rounded-full blur-sm"></div>
          <div className="absolute -top-1 -left-1 w-4 h-4 bg-green-900/50 rounded-full"></div>
          <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-900/40 rounded-full"></div>
          <div className="absolute -bottom-1 -left-2 w-3 h-3 bg-green-900/40 rounded-full"></div>
        </div>
      </div>
      ...
      <div className="absolute inset-0">
        ...
        {[...Array(6)].map((_, i) => {
          ...
          return (
            <div
              key={`ginger-${i}`}
              className="absolute w-6 h-3 bg-gradient-to-r from-amber-900/70 to-amber-800/60 rounded-full"
              style={{
                top: `${y}%`,
                left: `${x}%`,
        ...
        {/* Cinnamon Ring */}
        <div className="absolute inset-10 rounded-full border-2 border-amber-900/30"></div>
        ...
        {[...Array(12)].map((_, i) => {
          ...
          return (
            <div
              key={`clove-${i}`}
              className="absolute w-2 h-2 bg-brown-900/60 rounded-full"
              style={{
                top: `${y}%`,
                left: `${x}%`,
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <div className="relative">
          {/* Steam Rings */}
          <div className="absolute w-16 h-16 border border-amber-100/20 rounded-full blur-sm animate-ping"></div>
          <div className="absolute w-24 h-24 border border-amber-100/15 rounded-full blur-sm animate-ping" style={{animationDelay: '0.5s'}}></div>
          <div className="absolute w-32 h-32 border border-amber-100/10 rounded-full blur-sm animate-ping" style={{animationDelay: '1s'}}></div>
          
          {/* Central Steam */}
          <div className="w-12 h-12 bg-gradient-to-b from-amber-50/40 to-transparent rounded-full blur-md animate-pulse"></div>
        </div>
      </div>
      ...
      <div className="absolute inset-0">
        {[...Array(20)].map((_, i) => {
          ...
          return (
            <div
              key={`tea-dot-${i}`}
              className="absolute w-1 h-1 bg-amber-900/40 rounded-full"
              style={{
                top: `${y}%`,
                left: `${x}%`,
                animation: `twinkle ${2 + Math.random() * 2}s infinite ${i * 0.1}s`
              }}
            ></div>
          );
        })}
    ...
    <div className="absolute inset-0">
      <div className="absolute inset-4 rounded-full border-3 border-amber-900/40"></div>
      <div className="absolute inset-8 rounded-full border-2 border-amber-800/30"></div>
      <div className="absolute inset-12 rounded-full border border-amber-700/20"></div>
    </div>
  </div>
  
  {/* Cup Handle (Top View Perspective) */}
  <div className="absolute -right-6 top-1/2 transform -translate-y-1/2 w-8 h-32 bg-gradient-to-r from-amber-300/40 to-amber-400/30 rounded-l-2xl shadow-lg">
    <div className="absolute inset-y-4 left-2 w-1 bg-gradient-to-b from-amber-400/50 to-amber-500/40 rounded-full"></div>
  </div>
  
  {/* Cup Shadow */}
  <div className="absolute -inset-2 rounded-full bg-gradient-to-br from-amber-900/20 via-transparent to-amber-900/20 blur-md"></div>
</div>
