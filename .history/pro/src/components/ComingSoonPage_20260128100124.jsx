            ...
            <div className="relative mb-8">
              ...
              >
                ...
                <div className="relative mx-auto max-w-xs sm:max-w-sm">
                 ...
                 {/* Chai Cup - Top/Down View */}
<div className="relative bg-gradient-to-b from-amber-50/10 to-amber-100/5 rounded-full aspect-square shadow-2xl overflow-hidden border-2 border-amber-300/30 backdrop-blur-sm">
  
  {/* Cup Rim - Top View */}
  <div className="absolute inset-0 rounded-full border-8 border-amber-200/40"></div>
  
  {/* Inner Rim Shadow */}
  <div className="absolute inset-6 rounded-full bg-gradient-to-b from-amber-900/20 to-transparent"></div>
  
  {/* Chai Liquid Surface - Circular */}
  <div className="absolute inset-8 rounded-full bg-gradient-to-b from-amber-800 via-amber-700 to-amber-900 overflow-hidden">
    
    {/* Milk Foam/Cream Center */}
    <div className="absolute inset-4 rounded-full bg-gradient-to-b from-amber-100/50 via-amber-50/30 to-transparent">
      
      {/* Foam Bubbles Pattern */}
      <div className="absolute inset-0">
        {/* Center Large Bubble */}
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-12 h-12 bg-amber-50/40 rounded-full blur-sm"></div>
        
        {/* Concentric Circles of Bubbles */}
        {[1, 2, 3].map((ring) => (
          <div key={ring} className="absolute inset-0">
            {[...Array(ring * 6)].map((_, i) => {
              const angle = (i * 360) / (ring * 6);
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    animation: `pulse ${2 + Math.random() * 2}s infinite ${i * 0.1}s`
                  }}
                ></div>
              );
            })}
      ...
      {/* Spice Concentric Circles */}
      <div className="absolute inset-0">
        {/* Cardamom Seeds - Inner Ring */}
        {[...Array(8)].map((_, i) => {
          const angle = (i * 360) / 8;
          const radius = 25;
          const x = 50 + radius * Math.cos((angle * Math.PI) / 180);
          ...
          return (
            <div
              key={`cardamom-${i}`}
              className="absolute w-3 h-2 bg-gradient-to-r from-green-900/60 to-green-800/50 rounded-full transform rotate-45"
              style={{
                top: `${y}%`,
                left: `${x}%`,
        ...
        {[...Array(6)].map((_, i) => {
          ...
          return (
            <div
              key={`cinnamon-${i}`}
              className="absolute w-6 h-1.5 bg-gradient-to-r from-amber-900/70 to-amber-800/60 rounded-full"
              style={{
                top: `${y}%`,
                left: `${x}%`,
        ...
        {[...Array(12)].map((_, i) => {
          ...
          return (
            <div
              key={`clove-${i}`}
              className="absolute w-2 h-4 bg-gradient-to-b from-brown-900/60 to-brown-800/50 rounded-full"
              style={{
                top: `${y}%`,
                left: `${x}%`,
                transform: `translate(-50%, -50%) rotate(${angle}deg)`
      ...
      <div className="absolute inset-0">
        {[...Array(15)].map((_, i) => {
          ...
          return (
            <div
              key={`leaf-${i}`}
              className="absolute bg-amber-900/40 rounded-full"
              style={{
                top: `${y}%`,
                left: `${x}%`,
                width: `${size}px`,
                height: `${size}px`,
                transform: 'translate(-50%, -50%)',
                animation: `drift ${8 + Math.random() * 4}s infinite linear ${i * 0.3}s`
              }}
            ></div>
          );
        })}
      ...
      {/* Steam Rising from Center - Top View */}
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <div className="relative">
          {/* Concentric Steam Rings */}
          {[1, 2, 3].map((ring) => (
            <div
              key={`steam-ring-${ring}`}
              className="absolute rounded-full border border-amber-100/30 blur-sm"
              style={{
                width: `${ring * 40}px`,
                height: `${ring * 40}px`,
                top: '50%',
                left: '50%',
                transform: 'translate(-50%, -50%)',
                animation: `steamExpand ${2 + ring * 0.5}s infinite ease-out ${ring * 0.2}s`
              }}
            ></div>
          ))}
          ...
          <div className="relative">
            {[1, 2, 3].map((i) => (
              <div
                key={`steam-column-${i}`}
                className="absolute w-8 h-8 bg-gradient-to-b from-amber-50/50 to-transparent rounded-full blur-md"
                style={{
                  top: '50%',
                  left: '50%',
                  transform: 'translate(-50%, -50%)',
                  animation: `steamPulse ${1.5 + i * 0.3}s infinite ${i * 0.2}s`
                }}
              ></div>
            ))}
          </div>
      ...
      <div className="absolute inset-0">
        {[1, 2].map((ring) => (
          <div
            ...
            style={{
              margin: `${ring * 8}px`,
              animation: `ripple ${4 + ring}s infinite linear ${ring * 0.5}s`
            }}
          ></div>
        ))}
      </div>
    ...
    <div className="absolute inset-0">
      ...
      {/* Inner Ring */}
      <div className="absolute inset-8 rounded-full border border-amber-700/10"></div>
    </div>
  </div>
  
  {/* Handle - Top View Perspective */}
  <div className="absolute -right-4 top-1/2 transform -translate-y-1/2 w-12 h-24 bg-gradient-to-r from-amber-200/30 to-amber-300/20 rounded-l-2xl border-l border-t border-b border-amber-400/30 shadow-lg">
    {/* Handle Grip Details */}
    <div className="absolute inset-2 rounded-l-xl bg-gradient-to-r from-amber-300/20 to-amber-400/10"></div>
    <div className="absolute top-1/2 left-2 w-1 h-16 bg-gradient-to-b from-amber-400/30 to-amber-500/20 rounded-full transform -translate-y-1/2"></div>
  </div>
  
  {/* Cup Shadow/Depth */}
  <div className="absolute -inset-2 rounded-full bg-gradient-to-br from-amber-900/10 via-transparent to-amber-900/10 blur-md"></div>
  
  {/* Cup Base/Stand Shadow */}
  <div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 w-3/4 h-4 bg-gradient-to-t from-amber-900/20 to-transparent rounded-full blur-sm"></div>
</div>
...
<style jsx>{`
  ...
  @keyframes drift {
    ...
    25% {
      transform: translate(calc(-50% + 2px), calc(-50% - 1px)) rotate(90deg);
    }
    50% {
      transform: translate(calc(-50% + 1px), calc(-50% + 2px)) rotate(180deg);
    }
    75% {
      transform: translate(calc(-50% - 2px), calc(-50% + 1px)) rotate(270deg);
    }
    100% {
